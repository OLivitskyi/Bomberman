...
export default class BootMenu extends Component {
    ...
    async initialize(resolve, reject) {
        ...
        const title = new Component('h1', { id: "title" }, ["Enter the Gates of Doom"])
        const errorMessage = new Component('p', { id: "error-msg", style: "color: red" });
        const text = `What name shall echo in the abyss?`;
        const queryText = new Component('p', {}, [text])
        const message = new Component('span', {}, [`(max.${maxCharacters} symbols of fate)`])
        const username = new Input({ id: 'field', type: 'text', placeholder: 'Hero Name', name: 'boot-menu-username', value: "" })
